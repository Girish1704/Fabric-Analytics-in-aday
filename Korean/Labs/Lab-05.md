...
7. **시작 시간** 링크를 선택합니다.
    ...
    **참고:** 시작 시간은 상황에 따라 다릅니다.
    ...
    상세 정보 화면이 열립니다. 그러면 새로 고침에 대한 세부 정보가
    제공됩니다. 시작 시간, 종료 시간 및 기간이 나열됩니다. 또한 새로 고침된
    테이블/활동도 나열됩니다. 오류가 있는 경우 테이블/활동 이름을 클릭하여
    추가 조사를 할 수 있습니다.
...
11. **설정** 패널에 나타난 다음 세 가지 옵션을 확인할 수 있습니다
    ...
    있습니다. 또한 데이터 흐름의 담당자와 마지막 수정 시점을 확인할 수
    있습니다.
    
    - **보증 --** 이를 통해 데이터 흐름이 다른 사용자에게 표시될 때 **승격됨** 태그를 적용할지 **인증됨** 태그를 적용할지 지정할 수
    있습니다.
...
18. 예약된 새로 고침을 확인하고, 더 이상 필요하지 않은 경우 편집하거나
    ...
    제때 전달되지 않는 경우를 해결해야 합니다. 파이프라인을 사용하여 이
    문제를 해결해 보겠습니다.
...
3. 새 파이프라인 대화 상자가 열립니다. 파이프라인의 이름을
    ...
    **파이프라인 페이지로 이동되었습니다.** Azure Data Factory를 사용해 본
    ...
    실행 기록 보기
    등 주로 사용하는 작업 옵션을 확인할 수 있습니다. 또한 가운데 창에는
    파이프라인 구축을 시작하는 빠른 옵션이 있습니다.
...
13. 메뉴에서 **홈 -> 저장** 아이콘을 선택하여 파이프라인을 저장합니다.
    ...
    설정하는 것과 비교하여 파이프라인을 사용하면 다음과 같은 이점이
    있습니다.
...
4. **설명**을 다음과 같이 입력합니다. **"Iterator to refresh dataflow.
...
8. **파이프라인 식 작성기** 대화상자가 열립니다. 대화 상자 하단에
    ...
    a. **매개 변수:** 파이프라인에 전달되는 값입니다. 예: 하나의
    파이프라인에서 다른 파이프라인으로 전달된 값. 이러한 값은 모든
    표현식에서 사용할 수 있지만 파이프라인 실행 중에는 변경할 수
    없습니다.
...
17. **or** 함수에 두 번째 매개변수를 추가해야 합니다. 두 괄호의 끝
    사이에 **쉼표 하나를 추가**합니다. 이번에는 함수 이름을 입력해
    보겠습니다. **equ**를 입력하기 시작하면 사용 가능한 함수의
    드롭다운이 표시됩니다(IntelliSense라고 함). **equals** 함수를
    선택합니다.
...
6. **설명** 필드에 **"Dataflow activity to refresh
    df_People_Sharepoint dataflow"** 을 입력합니다.
...
4. **설명** 필드에 **"Set variable varIsSuccess to Yes"** 를
    입력합니다.
...
12. **확인**을 선택합니다. **반복기 설계 창**으로 되돌아갑니다.
    ...
    이제 데이터 흐름 활동이 실패할 경우 카운터를 설정해야 합니다.
    파이프라인에서는 변수를 자체 참조할 수 없습니다. 이는 카운터 변수
    varCounter의 값에 1을 추가하여 증가시킬 수 없음을 의미합니다(varCounter
    = varCounter + 1). 따라서 varTempCounter 변수를 사용합니다.
...
4. **설명** 필드에 **"Increment variable varTempCounter"** 를
    입력합니다.
...
4. **설명** 필드에 **"Wait for 300 seconds on 2nd try and 900 seconds
    on 3rd try"** 를 입력합니다.
...
11. 파이프라인 생성이 완료되었습니다. 상단 메뉴에서 **홈 -> 저장
...
1. **홈 -> 실행**을 선택하여 데이터 파이프라인을 테스트할 수 있습니다.
    ...
    **참고:** 데이터 파이프라인의 새로 고침이 완료되는 데 몇 분 정도 걸릴 수
    ...
    있습니다. 따라서 현재 파이프라인은 결코 실패하지 않습니다.

2. 일정에 따라 실행되도록 파이프라인을 설정할 수 있습니다. 상단

...
7. **開始時刻**のリンクを選択します。
    ...
    **注:** 開始時間はお客様によって異なります。
    ...
    [詳細]
    画面が開きます。これにより、更新の詳細が提供されます。開始時刻、終了時刻、期間が一覧表示されます。また、更新されたテーブルとアクティビティも一覧表示されます。失敗が発生した場合は、テーブルやアクティビティの名前をクリックしてさらに調査できます。
...
11. 表示された**設定**パネルに 3 つのオプションがあることがわかります。
    ...
    - **情報 --** データフローの名前を変更し、説明を追加できます。また、データフローの所有者と最終更新日時も確認できます。

    - **承認 --** データフローで他のユーザーが参照できる**昇格済み**または**認定済み**タグを付けるかどうかを指定できます。
...
ロジックを構築する必要があります。パイプラインを使ってこれを解決しましょう。
...
13. メニューから**ホーム ->
    ...
    (以前のデータフローで行ったように)
    スケジュールされた更新でデータフローを設定する場合と比較して、パイプラインを使用する利点に注目してください。
...
8. **パイプライン式ビルダー**
    ...
    a. **パラメーター:**
    パイプラインに渡される値。あるパイプラインから別のパイプラインに渡される値など。これらの値は任意の式で使用できますが、パイプライン実行の間に変更することはできません。
...
12. **OK** を選択します。**反復処理デザイン ペイン**に戻ります。
    ...
    次に、データフロー
    アクティビティが失敗した場合にカウンターを設定する必要があります。パイプラインでは、変数を自己参照することはできません。つまり、カウンター変数
...
11. これで、パイプラインの作成が完了しました。上部メニューから**ホーム
    -> 保存アイコン**を選択して、パイプラインを保存します。
...
1. パイプラインをテストするには、**ホーム -> 実行**を選択します。
    ...
    **注:**
    ...
    のファイルは常に利用可能です。したがって、パイプラインが失敗することはありません。

2. スケジュールに従って実行するようにパイプラインを設定できます。上部メニューから、**ホーム
    -> スケジュール**を選択します。[スケジュール]
    ダイアログが開きます。
...
11. 左側のパネルで Fabric ワークスペース **FAIAD_<ユーザー名>**
    ...
    **注**: [スケジュール] 画面には、(データフローのスケジュールのように)
    成功または失敗を通知するオプションはありません。通知は、パイプラインにアクティビティを追加することで実行できます。これはラボ環境であるため、このラボでは実行しません。

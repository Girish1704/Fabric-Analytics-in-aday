...
3. **データの取得 (3)** セクションで、**Dataflow Gen2 (4)**
    ...
    既定の名前のままにして、[Git 統合の有効化]
    がオンになっていることを確認します。次に**作成**を選択します。**データ
    フロー ページ**が表示されます。データ フロー (Gen2)
    のインターフェイスは、Power BI Desktop の Power Query
    に似ています。クエリは、Power BI Desktop からデータ フロー (Gen2)
...
9. **データフローのペイン**で、**Ctrl+V** を押します
    ...
    **VM ネイティブ クリップボード**を**有効**にします。ダイアログで [OK]
    ...
    ![P58#yIS1](../media/Lab-04/image9.png)
...
3. 推奨項目から **データフロー (Gen2) (3)** を選択します。
    ...
    既定の名前のままにして、[Git 統合の有効化]
    がオンになっていることを確認します。次に**作成**を選択します。"この名前のデータフローは既に存在します"
    というメッセージが表示される場合は、名前を **Dataflow 2**
    に変更します。データフロー
    ページが表示されます。ここまでデータフローについて説明してきました。次は、Power
...
9. **データフローのペイン**で**中央のペイン**を選択し、**Ctrl+V**
    ...
    ![P121#yIS1](../media/Lab-04/image22.png)
...
4. **[新しい接続] (1) を選択すると、**[接続設定]
    ダイアログが開きます。**環境**のドメインに
...
7. サインイン ダイアログ
    ...
    ![P182#yIS1](../media/Lab-04/image37.png)
...
これで、必要なすべてのデータがレイクハウスに取り込まれました。次のラボでは、SharePoint
データフローの更新スケジュールを設定します。

...
(Gen2)、パイプライン、DirectLake などの使い方がわかります。
...
1. 仮想マシンのデスクトップで **PowerBI Portal**
    を選択します。サインインを求めるメッセージが表示されることがあります。

    ![](../media/Lab-02/image6.png)

    ***注:**
    ラボ環境を使用している場合は、自動的にサインインされることがあります。*

    ***注:** Fabric が開かない場合は、ブラウザーで
    http://app.fabric.microsoft.com/ に移動します。*
...
9. Microsoft Fabric
    ...
    ***注:**
    既定のリージョンを変更しないでください。これはそのままにします。*
...
新しく作成したワークスペースに自動的に移動します。さまざまなデータ
ソースからレイクハウスにデータを取り込み、そのレイクハウスからのデータを使用してモデルを構築し、レポートを作成します。最初のステップでは、レイクハウスを作成します。これについては次に説明します。
...
3. 新しいレイクハウスのダイアログが表示されます。[名前]
    ...
    ***注:** プレビュー機能である**レイクハウス
    スキーマ**は非常に興味深い機能であり、知っておくべきものです。これは**プレビュー**状態であるため、ラボ環境に悪影響がないように無視します。機能が一般提供になったら、このラボに組み込みます。*
...
1. 画面左側のワークロード
    ...
    Data Warehouse が含まれます。詳しく見ていきましょう。
...
3. Data Factory のホーム ページが表示されます。以下は、Data Factory
    ...
    **項目の種類:**
    ...
    ワークフローを作成できます。
    これにより、データを構造化された方法で管理および処理しやすくなります。
    ...
    は、ワークフローをプログラムで作成、
    スケジュール、監視するために使用される、オープンソースのプラットフォームです。
    Data Factory でそれを使用して、複雑なデータ
    ワークフローを作成、スケジュール、
    管理できます。
    ...
    g. **ミラー化された SAP (プレビュー):** 既存の SAP 資産を Fabric
    の残りのデータとシームレスに統合します。

    h. **ミラー化された Oracle (プレビュー):** Fabric
    でのミラーリングにより、Oracle
    データベースが統合プラットフォームにレプリケートされ、他のデータ
    ソースとともに準リアルタイムで低遅延の分析が可能になります。

    i. **ミラー化された Google BigQuery (プレビュー):** Fabric
    でのミラーリングにより、Google BigQuery データを OneLake
    に継続的にレプリケートできるため、複雑な ETL が不要に
    なり、分析、AI、データ共有にまたがったシームレスな使用が可能になります。
    ...
    **作業の開始**
    ...
    c. **最初のパイプラインを作成する:**
    このセクションには、最初のパイプラインを作成する方法が示されています。パイプラインを使用すると、データ
...
2. Industry Solutions のホーム ページが表示されます。以下は、Industry
    ...
    **項目の種類:**
    ...
    b. **小売ソリューション:**
    膨大なデータの管理、さまざまなソースのデータ統合、意思決定を加速させるリアルタイム分析の提供に役立ちます。小売業者は、
    在庫の最適化、顧客のセグメント化、売上予測、動的な価格、不正の検出に、これらのソリューションを使用できます。
    ...
    b. **医療データ ソリューションの使用を開始する:** 医療データ
    ソリューションの展開を開始し、プロジェクトにそれを実装します。

    c. **持続可能性ソリューションについて学ぶ:** [詳細]
    ボタンをクリックして持続可能性ソリューションについて読み、プロジェクトでどのように使用できるのかを理解
    します。

    d. **持続可能性ソリューションの使用を開始する:**
    持続可能性ソリューションの展開を開始し、プロジェクトにそれを実装します。

    e. **小売ソリューションについて学ぶ:** [詳細]
    ボタンをクリックして小売ソリューションについて読み、プロジェクトでどのように使用できるのかを理解します。

    f. **小売ソリューションの使用を開始する:**
    小売ソリューションの展開を開始し、プロジェクトにそれを実装します。
...
2. Real-Time Intelligence のホーム
    ...
    **項目の種類:**
    ...
    g. **Anomaly Detector (プレビュー):** 異常検出は、イベントハウス
    テーブル内の異常なパターンと外れ値を自動的に識別します。

    h. **Operations エージェント (プレビュー):** Operation
    エージェントは、監視 ->分析 -> 決定 ->
    対応のサイクルを自動化します。継続的に重要なメトリックを追跡し、分析情報を取得して、特定の対象に対するアクションを推奨します。

    i. **マップ (プレビュー):** 地理空間の分析情報を Real-Time Intelligence
    に取り入れることで、だれでもイベントの発生場所を視覚化し、空間データを他の
    Fabric
    機能と統合して、よりスマートで場所を認識した意思決定ができるようになります。

    j. **デジタル ツイン ビルダー (プレビュー):** デジタル ツイン
    ...
    **作業の開始:**
    ...
    a. **リアルタイムでのエンドツーエンド エクスペリエンス:**
    [作業の開始] ボタンをクリックして、サンプル データ
    セットを使用してリアルタイム データ分析を探索します。
...
2. Data Warehouse のホーム ページが表示されます。以下は、Data Warehouse
    ...
    **Data Warehouse とは**
    ...
    l. **ミラーリングされた Azure SQL Managed Instance:**
    高可用性とディザスター リカバリーのために Azure SQL Managed Database
    をミラーリングするのに使用されます。
...
2. Databases のホーム ページが表示されます。以下は、Databases
    ...
    **項目の種類:**
    ...
    a. **SQL データベース:** Fabric の SQL データベースはデータベース
    ワークロードの一部であり、Fabric
    の他の項目からそのデータにアクセスできます。また、SQL
    ...
    b. **Cosmos DB:** Microsoft Fabric の Cosmos DB

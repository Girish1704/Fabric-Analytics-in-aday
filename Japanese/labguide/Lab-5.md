...
1.  左パネルでワークスペースを選択し、Fabric ワークスペース (**FAIAD\_\<ユーザー名\>**) に戻ります。
...
1.  左パネルでワークスペースを選択し、Fabric ワークスペース (**FAIAD\_\<ユーザー名\>**)
    に戻ります。
...
2.  このアクティビティを構成して、df_People_SharePoint データフローと接続します。下部ペインから、設定を選択します。
    ...
    > *注: 設定を表示するには、下部ペインを上にドラッグすることが必要な場合があります。* > > ![](../media/Lab-5/image22.png)


3.  **ワークスペース**がご自身の Fabric ワークスペース **FAIAD\_\<ユーザー名\>** に設定されていることを確認します。
...
1.  左側のパネルで **FAIAD\_\<ユーザー名\>** をクリックして、ワークスペースのホームに移動します。
...
1.  **varIsSuccess:** 種類は String、既定値は **No** に設定します。         この変数を使って、データフローの更新が成功したかどうかを表します。
...
2.  **システム変数:** いずれかのサービス内でエンティティを定義するときに、         式で使用できます。例: パイプライン ID、パイプライン名、トリガー名など。
...
11. 左側のパネルで Fabric ワークスペース **FAIAD\_\<ユーザー名\>** を選択して、
    ワークスペースに移動します。

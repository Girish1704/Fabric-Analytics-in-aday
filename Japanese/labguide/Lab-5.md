...
8. **パイプライン式ビルダー** ダイアログが開きます。ダイアログの下半分にはメニューが表示されます。
    ...
    a. **パラメーター:** パイプラインに渡される値。あるパイプラインから別のパイプラインに渡される値など。これらの値は任意の式で使用できますが、パイプライン実行の間に変更することはできません。
    ...
    f. **ライブラリ変数:** ライブラリ変数は、次で定義された変数を使用します: **変数ライブラリ Fabric 項目**。これらの変数は、ワークスペース全体で構成を管理するための一元的な方法を提供し、CI/CD ワークフローをサポートします。パイプラインやノートブック、レイクハウスのショートカットなどと併用できます。
...
23. 式は次のようになるはずです。 **@or(equals(variables('varCounter'),3),equals(variables('varIsSuccess'), variables('varSuccess')))**
...
11. 左側のパネルで Fabric ワークスペース **FAIAD_<inject key="Deployment ID" enableCopy="false"/>F** を選択して、
    ワークスペースに移動します。

...
1.  ラボ 2 のタスク 8 で作成した Fabric ワークスペース **FAIAD\_\<ユーザー名\> (1)** に戻りましょう。
...
3.  **データの取得 (3)** セクションで、**Dataflow Gen2 (4)** を選択します。
    ...
    既定の名前のままにして、\[Git 統合の有効化\] がオンになっていることを確認します。次に**作成**を選択します。**データ フロー ページ**が表示されます。データ フロー (Gen2) のインターフェイスは、Power BI Desktop の Power Query に似ています。クエリは、Power BI Desktop からデータ フロー (Gen2) にコピーすることができます。これを試してみましょう。
...
5.  先ほど作成したレイクハウスにテーブルを作成する必要があります。左パネルで、**レイクハウス -\> FAIAD\_\<ユーザー名\>** に移動します。
...
2.  左上隅の**保存と実行 (2)** を選択します。更新が開始されたという通知が表示されたら、データフローを閉じることができます **(3)**
    ...
    **注: FAIAD\_\<ユーザー名\>** ワークスペースが表示されます。データフローの実行が完了するまで、しばらくかかる場合があります。
...
3.  推奨項目から **データフロー (Gen2) (3)** を選択します。
    ...
    既定の名前のままにして、\[Git 統合の有効化\] がオンになっていることを確認します。次に**作成**を選択します。"この名前のデータフローは既に存在します" というメッセージが表示される場合は、名前を **Dataflow 2** に変更します。データフロー ページが表示されます。ここまでデータフローについて説明してきました。次は、Power BI Desktop からデータフローにクエリをコピーしましょう。
...
3.  \[データ変換先に接続\] ダイアログが開きます。**接続ドロップダウ**ンから **Lakehouse odl_user\_\<ユーザー名\> (なし)** を選択します。
...
6.  先ほど作成したレイクハウスにテーブルを作成する必要があります。左パネルで、**レイクハウス -\> FAIAD\_\<ユーザー名\>** に移動します。
...
4.  左上隅の**保存と実行 (1)** を選択します。更新が開始されたという通知が表示されたら、データフローを閉じることができます **(2)**
    ...
    **FAIAD\_\<ユーザー名\> ワークスペース**が表示されます。データフローが公開されるまで、しばらくかかる場合があります。

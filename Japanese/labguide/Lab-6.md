...
1. ラボ 2 のタスク 8 で作成した Fabric ワークスペース **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** に戻りましょう。

2. 選択する場合、**タスクフローを最小化して**項目のリスト全体を表示します。

3. レイクハウス、セマンティック モデル、SQL エンドポイントという 3 つの要素が lh_FAIAD に関連付けられていることがわかります。前のラボでは、レイクハウスについて説明し、SQL 分析エンドポイントを使用してビジュアル クエリを作成しました。左のナビゲーションで **FAIAD_<inject key="Deployment ID" enableCopy="false"/>** を選択し、**lh_FAIAD SQL 分析エンドポイント** オプションを選択してこのオプションの探索を続けます。エクスプローラーの **SQL ビュ**ーが表示されます。

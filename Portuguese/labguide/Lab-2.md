...
4. No campo **Nome**, insira **FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT** (encontrado na guia Ambiente).
...
3. Você será direcionado para a Página Inicial do Data Factory. A seguir, está uma explicação detalhada de suas seções, desenvolvidas para guiar você passo a passo no uso eficaz do Data Factory. Fluxo de Dados Gen2 é a próxima geração de Fluxo de Dados.
    ...
    **Tipos de item:**
    ...
    a. **Fluxo de dados Gen2:** os fluxos de dados são como receitas para transformar dados. Eles oferecem mais de 300 transformações diferentes que você pode aplicar aos seus dados. Isso significa que você pode limpar, combinar e alterar os dados de várias maneiras para atender às suas necessidades.

    b. **Pipeline:** pipelines são fluxos de trabalho que ajudam a automatizar os processos de dados. Eles permitem que você crie fluxos de trabalho de dados flexíveis que podem ser adaptados às suas necessidades específicas. Isso facilita o gerenciamento e o processamento de dados de forma estruturada.
    ...
    f. **Banco de dados espelhado:** um recurso para criar versões espelhadas de bancos de dados para backup, teste ou acesso somente leitura.

    g. **SAP Espelhado:** integre perfeitamente seu estado SAP existente com o restante de seus dados no Fabric.

    h. **Oracle Espelhado (versão preliminar):** um espelhamento no Fabric replica seus bancos de dados Oracle em uma plataforma unificada, permitindo análise quase em tempo real e de baixa latência ao lado de outras fontes de dados.

    i. **Google BigQuery Espelhado (versão preliminar):** o espelhamento no Fabric permite replicar continuamente dados do Google BigQuery no OneLake, eliminando o ETL complexo e permitindo o uso contínuo em análise, IA e compartilhamento de dados.

    j. **Biblioteca de variáveis:** contém uma lista de variáveis e seus valores padrão. Ela também pode conter outros conjuntos de valores com valores alternativos

    k. **Trabalho dbt (versão preliminar):** permite que você use o dbt para transformar dados com SQL em um ambiente familiar.
    ...
    **Introdução:**
    ...
    c. **Criar Seu Primeiro Pipeline:** essa seção orienta você sobre como criar seu primeiro pipeline. Os pipelines ajudam a automatizar e gerenciar os processos de dados de forma eficaz.
...
2. Você será direcionado para a Página Inicial do Industry Solutions. Veja a seguir uma visão geral detalhada de suas seções, desenvolvidas para ajudar você a usar o Industry Solutions de forma eficaz e passo a passo.
    ...
    **Introdução:** Para começar a usar o Industry Solutions, siga estas etapas:
    ...
    b. **Introdução às soluções de dados de Serviços de Saúde**: comece a implantar as soluções de dados de serviços de saúde e a implementá-las em seus projetos.
    ...
    d. **Introdução às soluções de Sustentabilidade:** comece a implantar as soluções de sustentabilidade e a implementá-las em seus projetos.
    ...
    f. **Introdução às soluções de Varejo**: comece a implantar as soluções de varejo e a implementá-las em seus projetos.

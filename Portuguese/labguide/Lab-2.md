...
3.  Uma caixa de diálogo Novo Lakehouse será exibida. Insira **lh_FAIAD** na caixa de texto Nome.
    ...
    > ***Observação:** "lh" refere-se a Lakehouse. Estamos prefixando "lh" para que seja fácil de identificar e pesquisar.  * > > ***Observação:** este recurso não está mais em versão prelimina**r, mas ainda não precisamos habilitá-lo.***
...
3.  Você será direcionado para a Página Inicial do Data Factory. A seguir, está uma explicação detalhada de suas seções, desenvolvidas para guiar você passo a passo no uso eficaz do Data Factory. Fluxo de Dados Gen2 é a próxima geração de Fluxo de Dados.
    ...
    **Tipos de item:**
    ...
    5)  **Copiar Trabalho:** é um recurso que permite copiar dados de uma fonte para outra. Ele fornece uma maneira simples e eficiente de mover dados entre diferentes armazenamentos.
    ...
    8)  **Oracle Espelhado (versão preliminar):** um espelhamento no Fabric replica seus bancos de dados Oracle em uma plataforma unificada, permitindo análise quase em tempo real e de baixa latência ao lado de outras fontes de dados.
    ...
    10) **Biblioteca de variáveis:** contém uma lista de variáveis e seus valores padrão. Ela também pode conter outros conjuntos de valores com valores alternativos

    11) **Trabalho dbt (versão preliminar):** permite que você use o dbt para transformar dados com SQL em um ambiente familiar.
...
2.  Você será direcionado para a Página Inicial do Industry Solutions. Veja a seguir uma visão geral detalhada de suas seções, desenvolvidas para ajudar você a usar o Industry Solutions de forma eficaz e passo a passo.
    ...
    **Tipos de item:**
    ...
    2)  **Soluções de varejo:** ajuda no gerenciamento de grandes volumes de dados, integrando dados de várias fontes e fornecendo análises em tempo real para tomada rápida de decisões. Os varejistas podem usar essas soluções para otimização de estoque, segmentação de clientes, previsão de vendas, preços dinâmicos e detecção de fraudes.

    3)  **Soluções de serviços de saúde:** são estrategicamente projetadas para acelerar o tempo de retorno para os clientes, atendendo à necessidade crítica de transformar com eficiência os dados de saúde em um formato adequado para análise.
...
2.  Você será direcionado para a Página Inicial do Real-Time Intelligence. A seguir, está uma visão geral detalhada de suas seções, desenvolvidas para ajudar você a usar o Real-Time Intelligence de forma eficaz e passo a passo.
    ...
    **Tipos de item:**
    ...
    j\. **Mapa (versão preliminar):** traga insights geoespaciais para o Real-Time Intelligence, permitindo que qualquer pessoa visualize onde os eventos acontecem, integre dados espaciais com outros recursos do Fabric e tome decisões mais inteligentes e sensíveis à localização.
...
2.  Você será direcionado para a Página Inicial do Data Warehouse. A seguir, está uma visão geral detalhada de suas seções, desenvolvidas para ajudar você a usar o Data Warehouse de forma eficaz e passo a passo.
    ...
    **Tipos de item:**
    ...
    g\. **Oracle espelhado (versão preliminar):** usado para espelhar o Oracle.
    ...
    l\. **Instância Gerenciada de SQL do Azure espelhado:** usada para espelhar os Bancos de Dados Gerenciados SQL do Azure para alta disponibilidade e recuperação de desastre.

    m\. **Banco de dados espelhado:** usado para replicar bancos de dados para alta disponibilidade e recuperação de desastres.

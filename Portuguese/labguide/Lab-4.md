...
1. Vamos voltar ao workspace do Fabric, **FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT (1)**, que você criou no Laboratório 2, Tarefa 8.
...
5. Queremos criar a tabela no Lakehouse que criamos anteriormente. No painel esquerdo, navegue para **Lakehouse -> FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT.**
...
2. No canto superior esquerdo, selecione **Salvar e Executar (1)**. Depois de ver a notificação de que uma atualização foi iniciada, você poderá fechar o fluxo de dados **(3)**
    ...
    > **Observação:** você será direcionado de volta para o **workspace FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT**. Pode levar alguns instantes para que a execução do Fluxo de Dados seja encerrada.
...
6. Quando terminar, feche a janela de configurações **(3)**.
    ...
    Você será direcionado de volta para o **workspace FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT**.
...
1. Vamos voltar ao workspace do Fabric, **FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT (1)**.
...
3. A caixa de diálogo Conectar ao destino de dados é aberta. Na **lista suspensa Conexão**, selecione **Lakehouse odl_user_<inject key="Deployment ID" enableCopy="false"/> (nenhum)**.
...
6. Queremos criar a tabela no Lakehouse que criamos anteriormente. No painel esquerdo, navegue para **Lakehouse -> FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT.**
...
4. No canto superior esquerdo, selecione **Salvar e Executar (1)**. Depois de ver a notificação de que uma atualização foi iniciada, você poderá fechar o fluxo de dados **(2)**
    ...
    Você será direcionado de volta para o **workspace FAIAD_<inject key="Deployment ID" enableCopy="false"/>_PT**. Pode levar alguns instantes para que Fluxo de Dados seja publicado.

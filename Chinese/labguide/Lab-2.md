...
3. 系统会将您定向到 Data Factory 主页。下面是其各部分的详细说明，旨在指导您逐步有效地使用 Data Factory。数据流 Gen2 是新一代数据流。
    ...
    **项目类型：**
    ...
    7)  **镜像的 SAP（预览版）：** 将现有 SAP 资产与 Fabric 中的其余数据无缝集成。

    8)  **镜像的 Oracle（预览版）：** Fabric 中的镜像将您的 Oracle 数据库复制到一个统一平台中，从而能够与其他数据源一起进行准实时、低延迟的分析。

    9)  **镜像的 Google Big Query（预览版）：** Fabric 中的镜像可让您将 Google BigQuery 数据持续复制到 OneLake 中，从而消除复杂的 ETL，并实现跨分析、AI 和数据共享的无缝使用。

    10) **变量库：包含变量及其默认值的列表。还可以包含保存备选值的其他值集**
...
2. 系统会将您定向到 Industry Solutions 主页。下面是其各部分的详细概述，旨在帮助您有效地逐步使用 Industry Solutions。
    ...
    **项目类型：**
    ...
    2)  **零售解决方案：**有助于管理大量数据，整合来自各个来源的数据，并提供实时分析以促进及时做出决策。零售商可以使用这些解决方案进行库存优化、客户细分、销售预测、动态定价和欺诈检测。

    3)  **医疗保健解决方案：**经过战略设计，旨在通过满足将医疗保健数据有效转换为适合格式以供分析的关键需求，加快客户实现价值的时间。
...
7. **异常检测器（预览版）：**异常情况检测可自动识别事件屋表中的异常模式和离群值。


8. **运营智能体（预览版）：**运营智能体自动执行观察 -> 分析 -> 决定 -> 行动周期。它们持续跟踪关键指标、提供见解并建议有针对性的操作。


9. **地图（预览版）：**将地理空间见解引入 Real-Time Intelligence，使任何人都能直观了解事件发生的位置，将空间数据与其他 Fabric 功能集成，并做出更明智的位置感知决策。


10. **数字孪生体生成器（预览版）：**数字孪生体生成器为用户提供低代码/无代码体验，以通过本体生成其业务概念（例如资产和流程）并对其进行建模。
...
5. **数据代理（预览版）：**用于创建对话式 AI 体验，以回答有关存储在湖屋、仓库、
...
9. **镜像的 Azure Cosmos DB：**用于镜像 Azure Cosmos DB。


10. **镜像的 SQL Server（预览版）：**用于镜像 SQL Server。


11. **镜像的 Azure SQL 托管实例：**用于镜像 Azure SQL 托管数据库以实现高可用性
    和灾难恢复。


12. **镜像的 Azure SQL 托管数据库：**用于镜像 Azure SQL 托管数据库以实现高可用性和灾难恢复。


13. **镜像的数据库（预览版）：**用于复制数据库以实现高可用性和灾难恢复。

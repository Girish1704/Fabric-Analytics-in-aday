...
3.  系统会将您定向到 Data Factory 主页。下面是其各部分的详细说明，旨在指导您逐步有效地使用 Data Factory。数据流 Gen2 是新一代数据流。
    ...
    **项目类型：**
    ...
    11) **dbt 作业（预览版）：**允许您使用 dbt 并将带有 SQL 的数据转换到熟悉的环境中。
...
2.  系统会将您定向到 Industry Solutions 主页。下面是其各部分的详细概述，旨在帮助您有效地逐步使用 Industry Solutions。
    ...
    **项目类型：**
    ...
    > **注意：**您可能看不到某些解决方案 > > **入门：** > 若要开始使用 Industry Solutions，请按照以下步骤操作：
...
10. **地图（预览版）：**将地理空间见解引入 Real-Time Intelligence，使任何人都能直观了解事件发生的位置，将空间数据与其他 Fabric 功能集成，并做出更明智的位置感知决策。
...
10. **像的 SQL Server**：用于镜像 SQL Server**。**


11. **镜像的 Azure Database for PostgreSQL：**用于镜像现有的 Azure Database for PostgreSQL.


12. **镜像的 Azure SQL 托管数据库：**用于镜像 Azure SQL 托管数据库以实现高可用性和灾难恢复。


13. **镜像的数据库：**用于复制数据库以实现高可用性和灾难恢复**。**

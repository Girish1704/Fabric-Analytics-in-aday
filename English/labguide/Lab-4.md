...
3.  Under the **Get Data (3)** section, select **Dataflow Gen2 (4).**
    ...
    Leave the default name. Then select **Create.** You will be navigated to the **Dataflow page**. The Dataflow Gen2 interface is like Power Query in Power BI Desktop. We can copy queries from Power BI Desktop into Dataflow Gen2. Let’s give this a try.
...
3.  Under Recommended items, select **Dataflow Gen2 (3)**.
    ...
    If you receive a message stating “A dataflow with this name already exists” then change the name to **Dataflow 2.** You will be navigated to the **Dataflow page**. Now that we are familiar with Dataflow, let’s go ahead and copy the queries from Power BI Desktop into Dataflow.
...
3.  Connect to data destination dialog opens. From the **Connection dropdown** select **Lakehouse odl_user\_\<username\> (none)**.
